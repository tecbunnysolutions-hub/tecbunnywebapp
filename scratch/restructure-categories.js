...
async function runRestructure() {
  try {
    ...
    for (const product of products) {
      const newCategory = mapProductToCategory(product.name);
      
      // Update only if the category has changed
      if (product.category !== newCategory) {
        ...
        if (updateError) {
          ...
          updatedCount++;
          console.log(`Updated [${product.name.slice(0, 35)}...] -> [${newCategory}]`);
        }
      }
    }

    console.log(`\nCategory Restructure Complete. Total products updated: ${updatedCount}/${products.length}`);
  } catch (err) {
    console.error('Error running restructure:', err);
  }
}
