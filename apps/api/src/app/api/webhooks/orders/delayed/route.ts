...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const isNewEvent = await claimWebhookEventId('webhook:order:delayed', eventId);
    if (!isNewEvent) {
      logger.info('Duplicate order delayed webhook event (Redis cache hit), skipping execution', { eventId, correlationId });
