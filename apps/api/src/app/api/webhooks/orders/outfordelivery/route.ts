import { createSupabaseServiceClient as createClient } from '@tecbunny/database/admin';
  ...
  try {
    ...
    if (timestampStr) {
      try {
        ...
        logger.error('Order out for delivery webhook timestamp validation failed', {
    ...
    if (existingEvent) {
      logger.info('Duplicate order out for delivery webhook event received, skipping execution', { eventId, correlationId });
      return NextResponse.json({ success: true, message: 'Event already processed (duplicate)' }, { status: 200 });
    }

    const result = await processOrderOutForDelivery(supabase, body, source);
    await logWebhookEvent(supabase, 'order_out_for_delivery', body, source, true, undefined, startTime, eventId);
