  ...
  try {
    ...
    if (timestampStr) {
      try {
        ...
        logger.error('Order delivered webhook timestamp validation failed', {
    ...
    if (existingEvent) {
      logger.info('Duplicate order delivered webhook event received, skipping execution', { eventId, correlationId });
      return NextResponse.json({ success: true, message: 'Event already processed (duplicate)' }, { status: 200 });
    }

    const result = await processOrderDelivered(supabase, body, source);
    await logWebhookEvent(supabase, 'order_delivered', body, source, true, undefined, startTime, eventId);
