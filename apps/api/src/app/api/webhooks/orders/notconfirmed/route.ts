...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const isNewEvent = await claimWebhookEventId('webhook:order:not-confirmed', eventId);
    if (!isNewEvent) {
      logger.info('Duplicate order not confirmed webhook event (Redis cache hit), skipping execution', { eventId, correlationId });
