...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const isNewEvent = await claimWebhookEventId('webhook:order:cancelled', eventId);
    if (!isNewEvent) {
      logger.info('Duplicate order cancelled webhook event (Redis cache hit), skipping execution', { eventId, correlationId });
