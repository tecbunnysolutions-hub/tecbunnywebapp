...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const isNewEvent = await claimWebhookEventId('webhook:customer:signup', eventId);
    if (!isNewEvent) {
      logger.info('Duplicate customer signup webhook event (Redis cache hit), skipping execution', { eventId, correlationId });
