...
export async function GET(request: NextRequest) {
  try {
    logger.info('analytics_dashboard.audit.requested');
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
    const { isAdmin } = await requireAdmin(user, supabase);

    if (!isAdmin) {
      logger.warn('analytics_dashboard.audit.unauthorized');
    ...
    logger.info('analytics_dashboard.audit.success', { range, topProductCount: topProducts.length });
    ...
    logger.error('analytics_dashboard.audit.failed', { error: error instanceof Error ? error.message : String(error) });
