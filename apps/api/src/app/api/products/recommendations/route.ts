...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info('products_recommendations.audit.requested', { hasUser: Boolean(userId), hasSession: Boolean(sessionId) });
    ...
    logger.info('products_recommendations.audit.success', {
      viewedCount: normalizedViewed.length,
      recommendedCount: normalizedRecommended.length,
    });
    ...
    logger.error('products_recommendations.audit.failed', { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ recentlyViewed: [], recommended: [] }, { status: 500 });
  }
}
