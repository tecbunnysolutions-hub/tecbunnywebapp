    ...
    for (const msg of results) {
      ...
      if (msg.message?.text) {
        ...
        if (content.type === 'TEXT') {
          ...
          textContent = `≡ƒôì Location: https://maps.google.com/?q=${content.latitude},${content.longitude} ${content.address ? `(${content.address})` : ''}`;
          ...
          textContent = `≡ƒæñ Contact: ${contactName} (${contactPhone})`;
        } else {
