...
export default function HomePage(_props: { initialPartnerBrands?: Array<{ name: string; logoUrl: string }>; initialHeroCarousel?: unknown }) {
  return <div className="bg-zinc-950 text-white">
    <section className="border-b border-zinc-800 bg-[radial-gradient(circle_at_80%_0%,rgba(37,99,235,.24),transparent_34%),linear-gradient(180deg,#10182a_0%,#09090b_85%)]">
      <div className="tb-container py-14 sm:py-20 lg:py-24"><div className="max-w-3xl">
        <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-400/30 bg-blue-400/10 px-3 py-1.5 text-sm font-semibold text-blue-200"><ShieldCheck size={16}/> Local technology team in Goa</p>
        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">Technology &amp; security solutions for your business.</h1>
        <p className="mt-5 max-w-2xl text-lg leading-relaxed text-zinc-300">TecBunny supplies, installs and supports computers, Wi-Fi, CCTV, access control and business IT for homes, shops, offices and hospitality.</p>
        <form action="/services" className="mt-8 flex max-w-2xl items-center gap-3 rounded-2xl border border-zinc-700 bg-zinc-900 p-2 shadow-2xl"><Search className="ml-2 shrink-0 text-zinc-400"/><input name="query" aria-label="Search TecBunny services" className="min-w-0 flex-1 bg-transparent px-1 py-3 text-base outline-none placeholder:text-zinc-500" placeholder="Search CCTV, laptop repair, Wi-Fi, AMC..."/><button className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold hover:bg-blue-500">Search</button></form>
        <div className="mt-5 flex flex-col gap-3 sm:flex-row"><Link href="/contact?intent=service_request&source=homepage_hero" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 font-bold hover:bg-blue-500">Get help <ArrowRight size={17}/></Link><Link href="#needs" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-zinc-700 px-5 font-bold hover:border-blue-400">Explore services</Link></div>
      </div></div>
    </section>
    <section id="needs" className="tb-container py-12 sm:py-16"><div className="mb-7"><p className="text-sm font-bold uppercase tracking-widest text-blue-400">Start here</p><h2 className="mt-2 text-3xl font-bold">What do you need today?</h2></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{categories.map(({ title, detail, href, icon: Icon }) => <Link key={title} href={href} className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-6 transition hover:-translate-y-1 hover:border-blue-500/60 hover:bg-zinc-900"><Icon className="mb-7 text-blue-400" size={30}/><h3 className="text-lg font-bold">{title}</h3><p className="mt-2 text-sm text-zinc-400">{detail}</p><span className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-blue-300">Explore <ArrowRight size={15}/></span></Link>)}</div></section>
