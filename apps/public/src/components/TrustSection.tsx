...
export function TrustSection({
  ...
  // Compact mode shows the three strongest trust points in a tighter layout.
  const points = variant === 'compact' ? TRUST_POINTS.slice(0, 3) : TRUST_POINTS;
  const isCompact = variant === 'compact';

  return (
    <section className={`${isCompact ? 'py-10 sm:py-14' : 'py-16 sm:py-24'} bg-zinc-950 border-y border-zinc-900 ${className}`}>
      <div className="container mx-auto px-6 max-w-screen-2xl">
        {/* Premium Intro Container */}
        <div className={`rounded-2xl border border-blue-500/20 bg-gradient-to-br from-blue-950/15 via-zinc-950/30 to-zinc-950 ${isCompact ? 'p-6 sm:p-8 mb-8' : 'p-8 sm:p-10 mb-14'}`}>
          <div className="space-y-4">
            ...
            <div>
              <h2 className={`${isCompact ? 'text-2xl sm:text-3xl' : 'text-3xl sm:text-4xl'} font-bold text-white font-tech tracking-tight`}>
                {title}
              </h2>
        ...
        <div className={`grid gap-6 sm:grid-cols-2 ${isCompact ? 'lg:grid-cols-3' : 'lg:grid-cols-3'}`}>
          {points.map((point) => {
