...
export function Logo({ className, width = 40, height = 40, alt = 'TecBunny Logo' }) {
    const [logoSrc, setLogoSrc] = React.useState(BRAND_LOGO_URL);
    React.useEffect(() => {
        let isMounted = true;
        const applyLogo = async (rawUrl) => {
    ...
    return (_jsx(Image, { src: logoSrc, alt: alt, width: width, height: height, className: `object-contain ${className ?? ''}`, priority: true, onError: () => {
