...
export const extractPincode = (payload) => {
    const candidates = [
...
const getAreaRecord = (relation) => {
    if (Array.isArray(relation))
        return relation[0] || {};
    return (relation && typeof relation === 'object') ? relation : {};
};
async function resolveAreaManager(pincode, role) {
    ...
    let eligibleIds = new Set((profiles || []).filter(profile => profile.role === role).map(profile => profile.id));
    ...
    for (const roleRow of roleRows || [])
        eligibleIds.add(roleRow.user_id);
...
async function notificationAlreadySent(sourceType, sourceId, notificationKind) {
...
const getOrderItems = (order) => {
    if (Array.isArray(order.items))
        return order.items;
    if (Array.isArray(order.items?.cart_items))
...
async function updateOrderAssignment(orderId, routing, role) {
    if (!isSupabaseServiceConfigured)
        return;
    ...
    if (error)
        logger.warn('area_notification.order_assignment_update_failed', { orderId, error: error.message });
}
async function updateTicketAssignment(ticketId, routing) {
    if (!isSupabaseServiceConfigured)
        return;
    const { error } = await createServiceClient()
        ...
        .eq('id', ticketId);
    if (error)
        logger.warn('area_notification.ticket_assignment_update_failed', { ticketId, error: error.message });
}
export async function sendOrderRoutingNotifications(order, role = 'sales_manager') {
...
export async function sendServiceTicketRoutingNotifications(ticket) {
