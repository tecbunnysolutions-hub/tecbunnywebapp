...
const formatOrderIdForDisplay = (orderData) => {
...
const hasHeaderControlChars = (value) => /[\r\n]/.test(value);
const assertSafeHeaderValue = (value, fieldName) => {
    if (value === undefined) {
        return;
    }
...
export class EmailService {
     ...
     */
    async sendTemplateEmail(to, templateType, templateData, options) {
        try {
     ...
     * Send custom email
     */
    async sendCustomEmail(to, subject, html, text, options) {
        try {
            assertSafeHeaderValue(to, 'to');
            assertSafeHeaderValue(options?.cc, 'cc');
            assertSafeHeaderValue(options?.bcc, 'bcc');
    ...
    async sendBulkEmails(emails, options) {
        ...
        for (let i = 0; i < emails.length; i += batchSize) {
            ...
            const batchPromises = batch.map(async (emailData) => {
                try {
                    const sent = await this.sendTemplateEmail(emailData.to, emailData.templateType, emailData.templateData);
                    if (sent) {
                        success++;
...
export const emailHelpers = {
    async sendEmailVerification(to, userName, otp) {
    ...
    async sendAdminOrderNotification(recipients, orderData) {
        const improvedEmailService = (await import('../improved-email-service')).default;
    ...
    async sendPaymentConfirmation(to, orderData, paymentData) {
        // Use improved email service instead of old one
        const improvedEmailService = (await import('../improved-email-service')).default;
        const displayOrderId = formatOrderIdForDisplay(orderData);
    ...
    async sendShippingNotification(to, orderData, shippingData) {
        const emailService = createEmailService();
    ...
    async sendPaymentFailed(to, orderData, paymentData) {
        const emailService = createEmailService();
    ...
    async sendPaymentPending(to, orderData, paymentData) {
        const emailService = createEmailService();
    ...
    async sendPickupNotification(to, orderData, pickupCode) {
        const emailService = createEmailService();
    ...
    async sendOrderCompletion(to, orderData) {
        const emailService = createEmailService();
    ...
    async sendPasswordResetOTP(to, userName, otp) {
        const emailService = createEmailService();
    ...
    async sendEmailChangeOTP(to, userName, otp) {
        const emailService = createEmailService();
