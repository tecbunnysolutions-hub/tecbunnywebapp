...
export class EnhancedCommissionService {
     ...
     */
    async getBlitzMultiplier(agentId) {
     ...
     */
    async calculateOrderCommission(orderId, agentId) {
        try {
            ...
            for (const item of orderItems) {
                const itemCommission = await this.calculateItemCommission(item, agentRules, preTaxAmount);
            ...
            const finalCommissionRate = await this.getFinalCommissionRate(agentId, preTaxAmount, agentRules);
            const calculation = {
                order_id: orderId,
                agent_id: agentId,
     ...
     */
    async saveCommissionRecord(calculation) {
        try {
            const supabase = this.supabase;
            if (!supabase || !isSupabaseServiceConfigured) {
     ...
     */
    async triggerAgentWhatsApp(calculation) {
        try {
            const supabase = this.supabase;
            if (!supabase)
                return;
     ...
     * Get agent commission rules
     */
    async getAgentCommissionRules(agentId) {
        try {
            const supabase = this.supabase;
            if (!supabase || !isSupabaseServiceConfigured) {
     ...
     */
    async calculateItemCommission(item, agentRules, _orderTotal) {
        ...
        if (!applicableRule) {
            ...
            applicableRule = agentRules.find(rule => rule.product_category && !rule.product_id);
        }
        if (!applicableRule) {
            // Use default rule (no specific product or category)
            applicableRule = agentRules.find(rule => !rule.product_id && !rule.product_category);
        }
     ...
     */
    async getFinalCommissionRate(agentId, orderTotal, rules) {
        // Find rules that apply to the entire order (bonus rates for high-value orders)
        const orderLevelRules = rules.filter(rule => !rule.product_id &&
     ...
     * Update agent points balance
     */
    async updateAgentPoints(agentId, commissionAmount) {
        try {
            const supabase = this.supabase;
            if (!supabase || !isSupabaseServiceConfigured) {
     ...
     */
    async getAgentCommissions(agentId, startDate, endDate, status) {
        try {
            const supabase = this.supabase;
            if (!supabase || !isSupabaseServiceConfigured) {
     ...
     */
    async awardCommission(orderId, agentId, _orderTotal) {
        try {
            const supabase = this.supabase;
            if (!supabase || !isSupabaseServiceConfigured) {
     ...
     */
    async processCommissionPayment(commissionIds, _paymentDetails) {
        try {
            const supabase = this.supabase;
            if (!supabase || !isSupabaseServiceConfigured) {
