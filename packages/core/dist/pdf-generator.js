...
async function fetchBoundedBuffer(url, label) {
...
function truncateQuoteText(value, maxLength) {
...
export async function buildPdf(options) {
    ...
    if (fontRegularBuffer) {
        try {
            // @ts-ignore - fontkit types are not available
    ...
    const drawWrappedText = (text, x, y, maxWidth, font, fontSize, color) => {
    ...
    const items = Array.isArray(selections?.items)
