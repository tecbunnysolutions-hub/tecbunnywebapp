'use server';
import { getGstRatesFromDb, getRolePermissionsFromDb, getCustomerCategoriesFromDb, getAppSettings } from './config-db';
/**
...
export async function hasPermission(role, permission) {
    ...
    if (!rolePerms)
        return false;
    if ('canManageEverything' in rolePerms)
        return true;
 ...
 */
export async function getCustomerBenefits(category) {
