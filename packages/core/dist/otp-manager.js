...
const legacyOtpStorage = new Map();
/**
 * Distributed OTP fallback using Redis when Supabase is not available
 */
async function setDistributedOTP(id, record) {
    try {
...
async function getDistributedOTP(id) {
    try {
        const redis = getRedis();
        if (redis) {
...
export async function assertOtpVelocity(phoneNumber, ipAddress = 'unknown') {
    const redis = getRedis();
    if (!redis) {
...
export class OTPManager {
    ...
    hashOTP(code, salt = 'tecbunny_static_salt') {
    ...
    async sendWhatsAppOTP(phone, code, purpose) {
        try {
    ...
    async sendOTPViaChannel(channel, phone, email, code, purpose) {
        if (channel === 'email')
    ...
    async verifyOTP(arg1, arg2, arg3) {
        if (typeof arg1 === 'object' && arg1 !== null) {
            // New verification path (OTPVerification)
            const verification = arg1;
    ...
    async storeOTP(email, otp, type = 'signup') {
        ...
        try {
            ...
            const columnMissing = typeof error?.message === 'string' &&
    ...
    storeOTPInMemory(email, otp, type) {
    ...
    verifyOTPFromMemory(email, otp, type) {
        const key = `${email}:${type}`;
    ...
    async resendOTPWithFallback(otpId, fallbackChannel) {
        try {
            if (!supabase) {
