...
export const AuthProvider = ({ children }) => {
    ...
    const cartSyncLockRef = useRef(false);
    useEffect(() => {
    ...
    const firstLoginAttemptedRef = useRef(new Set());
    ...
    const syncGuestCartToUser = useCallback(async (userId, abortSignal) => {
        if (cartSyncLockRef.current)
            return;
    ...
    const setWishlistOwner = useCallback(async (ownerKey) => {
        try {
    ...
    const buildFallbackProfile = useCallback((supabaseUser) => {
        const appMetadataRole = extractRoleFromMetadata(supabaseUser.app_metadata);
        ...
        return {
            ...
            permissions: Array.isArray(supabaseUser.app_metadata?.permissions) ? supabaseUser.app_metadata.permissions : []
        };
    }, []);
    const triggerFirstLoginWhatsApp = useCallback(async (profile) => {
    ...
    const fetchUserProfile = useCallback(async (supabaseUser) => {
        ...
        const appMetadataRole = extractRoleFromMetadata(supabaseUser.app_metadata);
        const userMetadataRole = extractRoleFromMetadata(supabaseUser.user_metadata);
        const resolvedRole = appMetadataRole ?? userMetadataRole ?? 'customer';
        const MAX_RETRIES = 5;
        const INITIAL_DELAY_MS = 200;
        const queryProfile = async (retryCount = 0) => {
            try {
                ...
                let addressObj = profile.address;
    ...
    const signup = async (details) => {
        try {
            ...
            const preferredChannel = details.preferredChannel && ['email', 'whatsapp'].includes(details.preferredChannel)
            ...
            if (!response.ok) {
                ...
                if (isCaptchaError && process.env.NODE_ENV !== 'production') {
                    try {
                        const retryHeaders = { 'Content-Type': 'application/json', 'x-bypass-captcha': '1' };
    ...
    const resendConfirmation = async (email, captchaToken) => {
        try {
            ...
            let storedMobile = undefined;
            if (stored) {
                try {
                    storedMobile = JSON.parse(stored).mobile;
                }
                catch { }
            }
