...
export const OrderProvider = ({ children }) => {
    ...
    const hydrateCartItemsWithProductData = useCallback(async (items) => {
        ...
        const ids = Array.from(new Set(itemsNeedingLookup
        ...
        return items.map(item => {
            ...
            const resolvedHsn = productRecord.hsnCode ??
                productRecord.hsn_code ??
                productRecord.hsn ??
                productRecord.hsn_sac ??
                null;
            const resolvedGst = productRecord.gstRate ??
    ...
    const createOrder = useCallback(async (orderData) => {
        ...
        try {
            ...
            const hydratedItems = await hydrateCartItemsWithProductData(itemsToProcess);
            // Calculate totals
            ...
            const orderPayload = {
                ...
                discount_amount: Math.round((orderData.discount_amount || 0) * 100) / 100,
                shipping_amount: Math.round((orderData.shipping_amount || 0) * 100) / 100,
            ...
            const hydratedOrder = {
                ...
                pickup_store: createdOrder.pickup_store ?? orderData.pickup_store ?? undefined,
                delivery_address: createdOrder.delivery_address ?? orderData.delivery_address ?? undefined,
    ...
    const getOrders = useCallback(async (customerId) => {
        try {
            ...
            if (customerId) {
                if (user && (user.email || user.mobile)) {
                    ...
                    if (user.mobile)
                        conditions.push(`customer_phone.eq.${user.mobile}`);
    ...
    return _jsx(OrderContext.Provider, { value: value, children: children });
};
