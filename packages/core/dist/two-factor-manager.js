...
class TwoFactorManager {
    resolveSupabaseClient(supabase) {
        if (supabase) {
            return supabase;
        }
    ...
    generateSecret(email) {
        ...
        const backupCodes = Array.from({ length: 10 }, () => randomBytes(4).toString('hex').toUpperCase().match(/.{1,4}/g)?.join('-') || '');
        return {
    ...
    decryptSecret(storedSecret) {
        ...
        const decipher = createDecipheriv('aes-256-gcm', this.getEncryptionKey(), Buffer.from(ivValue, 'base64url'));
    ...
    verifyBackupCodeHash(storedCode, normalizedCode) {
    ...
    verifyBackupCode(backupCodes, usedCodes, code) {
        ...
        return backupCodes.some((backupCode) => this.verifyBackupCodeHash(backupCode, normalizedCode) &&
            !usedCodes.includes(backupCode));
    }
    // Mark backup code as used
    markBackupCodeUsed(backupCodes, usedCodes, code) {
        const normalizedCode = code.replace(/-/g, '').toUpperCase();
        const matchingCode = backupCodes.find(backupCode => this.verifyBackupCodeHash(backupCode, normalizedCode));
    ...
    async enableTwoFactor(userId, secret, backupCodes, supabase) {
    ...
    async disableTwoFactor(userId, supabase) {
        const client = this.resolveSupabaseClient(supabase);
        try {
            const { error } = await client
                .from('profiles')
    ...
    async getTwoFactorStatus(userId, supabase) {
        let client;
        try {
    ...
    async verifyTwoFactor(userId, token, supabase) {
        let client;
        try {
            client = this.resolveSupabaseClient(supabase);
        }
        ...
        try {
            ...
            if (backupCodes.length > 0 &&
                ...
                await client
                    .from('profiles')
                    .update({ two_factor_backup_codes_used: updatedUsedCodes })
                    .eq('id', userId);
                return {
                    success: true,
