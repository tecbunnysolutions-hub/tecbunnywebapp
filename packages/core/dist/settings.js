 ...
 */
export async function getDefaultGstPercentage() {
    const envVal = process.env.NEXT_PUBLIC_GST_RATE;
    if (envVal) {
        const parsed = parseFloat(envVal);
        if (!isNaN(parsed))
            return parsed * 100; // e.g. 0.18 -> 18
    }
 ...
 */
export async function resolveProductGstRate(productId, dbGstRate, dbHsnCode) {
    try {
 ...
 */
export async function getPolicyContent(pageKey, defaultTitle) {
    try {
        const supabase = await getSupabaseClient();
        const { data, error } = await supabase
            .from('policies')
