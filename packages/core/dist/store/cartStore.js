...
const CART_STORAGE_NAMES = ['cart', 'appliedCoupon', 'cartLastUpdated', 'abandonedEmailSent'];
const resolveHsnCode = (value) => {
...
const normalizeCartItem = (item) => {
    const normalizedHsn = resolveHsnCode(item?.hsnCode ?? item?.hsn_code ?? item?.hsn ?? item?.hsn_sac ?? item?.hsnsac ?? item?.hsnSac);
...
const removeCartStorageForUser = (user) => {
    CART_STORAGE_NAMES.forEach((key) => localStorage.removeItem(getStorageKey(key, user)));
};
const readStorageWithLegacyFallback = (key, user) => {
...
const isGuestSessionExpired = (user) => {
    if (user)
        return false;
...
const initializeGuestSession = (user) => {
    if (user)
        return;
...
export const useCartStore = create((set, get) => ({
    ...
    saveCartToStorage: (user) => {
        if (user?.role === 'superadmin')
            return;
        const state = get();
        if (!state.isHydrated || state.isMergingAccountCart)
            return;
        if (!user && isGuestSessionExpired(user)) {
            set({ isSessionExpired: true });
            return;
    ...
    checkAbandonedCart: (user) => {
        const state = get();
        if (!state.isHydrated || !user || state.isSessionExpired)
            return;
    ...
    addToCart: (item, quantity = 1, user, trackEvent) => {
        ...
        set((state) => {
            ...
            let newItems;
            if (existingItem) {
                newItems = state.cartItems.map((cartItem) => cartItem.id === item.id
    ...
    mergeGuestCartWithUserCart: async (userId, supabaseClient) => {
        if (userId === 'superadmin-root-id')
            return;
        try {
            ...
            const addItemToMerge = (item) => {
                if (!item?.id)
                    return;
                const normalizedItem = normalizeCartItem(item);
                const existing = mergedMap.get(normalizedItem.id);
                if (existing) {
                    const isServiceItem = normalizedItem.product_type === 'service' ||
