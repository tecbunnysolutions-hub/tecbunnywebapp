...
const CODE_LOOKUP = new Map(Object.values(STATE_ALIASES).map((state) => [state.code, state]));
...
export function resolveIndianStateInfo(value) {
...
export function resolveIndianStateFromText(value) {
...
export function formatPlaceOfSupply(stateInfo, fallbackState) {
    if (stateInfo) {
        return `${stateInfo.code} - ${stateInfo.name}`;
    }
    return fallbackState?.trim() || 'Not captured';
}
export const TECBUNNY_REGISTERED_STATE = { code: '30', name: 'Goa' };
