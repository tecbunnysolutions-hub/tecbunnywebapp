...
function roleInherits(actual, required, seen = new Set()) {
    if (actual === required)
        return true;
    if (seen.has(actual))
        return false;
...
export function isAtLeast(actual, required) {
    if (!ALL_ROLES.includes(actual) || !ALL_ROLES.includes(required))
        return false;
...
function collectPermissions(role, result = new Set()) {
    for (const parent of ROLE_PARENTS[role])
        collectPermissions(parent, result);
    for (const permission of DIRECT_PERMISSIONS[role])
        result.add(permission);
    return result;
}
export const EFFECTIVE_PERMISSIONS = Object.fromEntries(ALL_ROLES.map((role) => [role, collectPermissions(authorizationRole(role))]));
export function permissionImplies(granted, required) {
    if (granted === required || granted === '*')
        return true;
    const grantedParts = granted.split(':');
    const requiredParts = required.split(':');
    if (grantedParts.at(-1) === 'all' && grantedParts[0] === requiredParts[0])
        return true;
    return grantedParts.length === requiredParts.length
        && grantedParts.every((part, index) => part === '*' || part === requiredParts[index]);
}
export function hasPermission(role, permission) {
    if (authorizationRole(role) === 'superadmin')
        return true;
