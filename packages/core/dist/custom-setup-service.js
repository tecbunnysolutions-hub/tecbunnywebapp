...
export async function fetchCustomSetupTemplateBySlug(slug) {
    if (!slug) {
        return null;
    }
    try {
...
export function buildCustomSetupBlueprintSummary(template) {
    if (!template) {
        return null;
    }
    const variables = Array.isArray(template.variables)
        ...
        : [];
    const systems = sortSystems(template.systems).map((system) => {
...
export async function getCustomSetupBlueprintSummary(slug) {
