...
import { generatePayuHash, getPayuPaymentUrl, normalisePayuEnvironment } from '../payu-service';
...
export class PaymentService {
    ...
    resolveEnvironmentPreference(envs) {
    ...
    async initiatePayuPayment(params) {
        ...
        const hasService = cartItemsList.some((item) => String(item.productId || item.id).startsWith('service-') ||
        ...
        const hash = generatePayuHash({ merchantKey, merchantSalt, environment }, payuPayload);
        const paymentParams = {
            key: merchantKey,
            txnid: txnId,
            amount,
    ...
    async updatePaymentStatus(params) {
        ...
        let newPaymentStatus = order.payment_status ?? null;
        switch (status) {
            case 'success':
