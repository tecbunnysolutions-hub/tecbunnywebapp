...
export class OrderService {
    ...
    async createOrder(params) {
        ...
        const normalizeOrderType = (value) => {
            ...
            if (['pickup', 'pick-up', 'store pickup', 'store-pickup'].includes(key))
                return 'Pickup';
            if (['walk-in', 'walkin', 'walk in'].includes(key))
                return 'Walk-in';
            if (['service', 'service order', 'service_call', 'service-call'].includes(key))
                return 'Service';
            if (['repair', 'repair order', 'rma'].includes(key))
                return 'Repair';
            if (['installation', 'install', 'installation order'].includes(key))
                return 'Installation';
            if (['setup', 'set-up', 'custom setup', 'customised setup'].includes(key))
                return 'Setup';
        ...
        const pickupStore = orderType === 'Pickup'
            ...
            : null;
        const placeOfSupply = formatPlaceOfSupply(destinationState, typeof orderData.place_of_supply === 'string' ? orderData.place_of_supply : orderData.customer_state);
        ...
        if (orderData.agent_id) {
            try {
                if (isAgentThemselves) {
                    const commissionResult = await enhancedCommissionService.calculateOrderCommission(createdOrder.id, orderData.agent_id);
