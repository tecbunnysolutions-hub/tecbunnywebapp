...
import { ALL_ROLES, hasPermission, isAtLeast, normalizeRole, permissionImplies } from './roles';
...
const fetchProfileRole = async (supabase, userId) => {
    try {
...
export async function getServerAuthState() {
    ...
    const resolvedRole = metadataRole ?? (await fetchProfileRole(supabase, user.id)) ?? DEFAULT_ROLE;
    return { supabase, session, role: resolvedRole, permissions };
}
export const roleMatches = (role, options) => {
...
export async function requireApiRole(options = {}) {
    ...
    return { supabase, session, role, permissions };
}
// New utility to evaluate specific permissions
export async function hasServerPermission(requiredPermission) {
...
const REGION_SCOPED_ROLES = new Set([
    'sales_executive',
    'store_executive',
    'sales_agent',
    'service_engineer',
 ...
 */
export async function requireAreaPermission(requiredPermission, areaId) {
    ...
    if (!REGION_SCOPED_ROLES.has(auth.role)) {
        return { ...auth, areaId: null };
    }
    if (!areaId) {
        return { error: NextResponse.json({ error: 'Forbidden: Resource area is required' }, { status: 403 }) };
    }
    const { data, error } = await auth.supabase
        ...
        .maybeSingle();
    if (error || !data) {
        return { error: NextResponse.json({ error: 'Forbidden: Cross-region access denied' }, { status: 403 }) };
    }
    return { ...auth, areaId };
}
