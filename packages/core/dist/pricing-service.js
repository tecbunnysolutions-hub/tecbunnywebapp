...
export class PricingService {
     ...
     */
    async getProductPrice(product, context) {
     ...
     */
    async getB2CPrice(product, context) {
        const basePrice = Number(product.price) || 0;
        const originalPrice = Number(product.mrp) || basePrice;
        ...
        if (context.customer_category) {
            ...
            const category = categories[context.customer_category];
     ...
     */
    async getB2BPrice(product, context, basePrice, originalPrice) {
     ...
     */
    async getCustomerPricingContext(userId) {
        if (!userId) {
            return {
                customer_type: 'B2C',
     ...
     */
    async calculateCartTotal(items, context) {
        let subtotal = 0;
        let totalDiscount = 0;
        const itemPrices = [];
        ...
        for (const item of items) {
            ...
            const priceInfo = await this.getProductPrice(verifiedProduct, { ...context, quantity: item.quantity });
     ...
     */
    async upgradeToB2B(userId, gstinDetails) {
