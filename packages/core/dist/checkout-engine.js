...
import { resolveIndianStateInfo } from './indian-tax';
const toPaise = (value) => Math.round((Number.isFinite(value) ? value : 0) * 100);
...
export class CheckoutEngine {
    ...
    async calculate(request) {
        ...
        try {
            ...
            const discountResult = await offerDiscountService.calculateCartPricing(pricedItems, pricingContext.customer_category, appliedCoupon || undefined);
            ...
            const distributedDiscounts = (() => {
                ...
                if (totalGross <= 0 || totalDiscountApplied === 0)
                    return itemsToDistribute.map(() => 0);
            ...
            if (salesAgentId) {
                try {
                    ...
                    const { data: agent } = await pricingService['getSupabaseClient']().then(s => s.from('sales_agents').select('commission_rate').eq('id', salesAgentId).single()).catch(() => ({ data: null }));
