...
const taxClassificationSchema = z.object({
    ...
    gst_rate: z.coerce.number().refine((value) => GST_TIERS.includes(value), 'GST rate must be one of 0, 5, 12, 18, or 28'),
...
export function parseTaxClassification(raw) {
    let parsed;
    try {
    ...
    if (!result.success) {
        throw new TaxClassificationError(`Gemini tax classification failed schema validation: ${result.error.issues.map(issue => issue.message).join('; ')}`, 422);
    }
    return {
        hsn_code: result.data.hsn_code,
        gst_rate: result.data.gst_rate,
        confidence_score: Math.round(result.data.confidence_score * 100) / 100,
        justification: result.data.justification.trim(),
    };
}
function buildTaxPrompt(input) {
    const product = {
...
export async function classifyProductTax(input, correlationId) {
