...
function getQuoteActionSecret() {
    const secret = (process.env.QUOTE_ACTION_TOKEN_SECRET ||
...
export function createQuoteActionToken(quoteId, scope, ttlSeconds = DEFAULT_TTL_SECONDS) {
...
export function verifyQuoteActionToken(token, quoteId, allowedScopes) {
    ...
    try {
        ...
        const now = Math.floor(Date.now() / 1000);
        return (payload.quoteId === quoteId &&
            typeof payload.exp === 'number' &&
            payload.exp > now &&
            typeof payload.scope === 'string' &&
            allowedScopes.includes(payload.scope));
    }
    catch {
        return false;
    }
