...
export async function isUserAdmin(user, supabase) {
...
export async function isUserSuperadmin(user, supabase) {
    if (await isSuperadminSession()) {
        return true;
    }
    const metadataRole = normalizeRole(user.app_metadata?.role);
...
export async function requireSuperadmin(user, supabase) {
    if (await isSuperadminSession()) {
