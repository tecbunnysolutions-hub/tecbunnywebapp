...
class ImprovedEmailService {
    ...
    config;
    lastSendTime = 0;
    isMainServiceDown = false;
    isBackupServiceDown = false;
    lastMainServiceCheck = 0;
    lastBackupServiceCheck = 0;
    constructor() {
        this.config = {
    ...
    async verifyConnections() {
        try {
            if (!this.transporter)
                throw new Error('Primary transporter not configured');
    ...
    async sendWithTransporter(transporter, options, transporterName) {
        try {
            const headerValues = [
