...
class EnvironmentValidator {
  ...
  private validateEnvironment() {
    ...
    let serviceEnv: any = {};
    try {
      publicEnv = requireSupabasePublicEnv();
      serviceEnv = requireSupabaseServiceEnv();
    } catch (e) {
      // Ignored here; Zod will catch the missing url/anonKey below if they are missing
    }
    ...
    const unvalidated = {
      supabase: {
        ...
        publicKeySource: publicEnv.keySource || 'anon',
    ...
    if (!result.success) {
      ...
      if (process.env.NODE_ENV !== 'production' && !process.argv[1]?.endsWith('environment-validator.ts')) {
        logger.info('Environment validation completed', { nodeEnv: this.config.app?.nodeEnv });
      }
    }
  }
  ...
  getFeatureStatus(): Record<string, boolean> {
    const smtp = (this.config as EnvironmentConfig).smtp;
    const whatsapp = (this.config as EnvironmentConfig).whatsapp;
    return {
      email: Boolean(smtp?.host && smtp?.user && smtp?.pass),
      whatsapp: Boolean(whatsapp?.apiKey && whatsapp?.baseUrl),
    };
  }
}
...
if (typeof process !== 'undefined' && process.argv && process.argv[1] && process.argv[1].endsWith('environment-validator.ts')) {
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (!validator.isValid()) {
      console.error('❌ Environment validation failed. Missing/Invalid variables:');
