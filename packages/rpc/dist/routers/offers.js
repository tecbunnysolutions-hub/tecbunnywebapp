...
function isMissingOffersRelation(error) {
...
function inferDiscountType(row) {
    ...
    if (row.discount_amount !== undefined && row.discount_amount !== null)
        return 'fixed_amount';
    if (fromLegacyType.includes('percentage'))
        return 'percentage';
    if (fromLegacyType.includes('amount') || fromLegacyType.includes('fixed'))
        return 'fixed_amount';
    if (fromLegacyType.includes('shipping'))
        return 'free_shipping';
    return 'percentage';
}
function mapLegacyOffer(row) {
...
function applyOfferFilters(offers, filters) {
    const now = new Date();
...
const validateOfferBusinessRules = (payload) => {
